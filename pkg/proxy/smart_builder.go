...
func (b *SmartBuilder) Build(configName string, targetURL *url.URL, passHostHeader, preservePath bool, flushInterval time.Duration) (http.Handler, error) {
	...
	//
	// The fast proxy also dials with its own net.Dialer, which would bypass a
	// tailnet-bound transport and reach the backend over the host network, so
	// those always take the httputil proxy.
	if serversTransport.Tailnet != "" || targetURL.Scheme == "h2c" || (targetURL.Scheme == "https" && !serversTransport.DisableHTTP2) {
