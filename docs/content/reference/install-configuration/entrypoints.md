...
| <a id="opt-http3-allow0rtt" href="#opt-http3-allow0rtt" title="#opt-http3-allow0rtt">`http3.allow0RTT`</a> | Contols whether QUIC 0RTT handshakes are permitted. <br /> This defaults to false because enablement allows for the bypassing of IP allow-lists.                                                                                                                                                                                                                                                                                                                                                                                                           | false                       | No       |
| <a id="opt-http3-initialPacketSize" href="#opt-http3-initialPacketSize" title="#opt-http3-initialPacketSize">`http3.initialPacketSize`</a> | Sets the initial size and the lower limit for packets sent, meausred in bytes. Values below 1200 are invalid as per the QUIC specifications. If left unset then this value will be automatically managed by the underlying QUIC libraries. This setting can be necessary to tweak in network situations where MTU limits interfere early in the QUIC handshake and PMTU discovery can not engage. | -                       | No       |
...
!!! warning "Security"
    ...
    Setting `allow0RTT` to true (default is `false`) allows for the bypassing of IP allow-lists during QUIC 0-RTT handshakes.
    Ensure that you are not reliant upon such allow-lists when enabling this feature. More information can be [found here](https://github.com/traefik/traefik/security/advisories/GHSA-gxrv-wf35-62w9)
