...
| <a id="opt-serverstransport-tailnet" href="#opt-serverstransport-tailnet" title="#opt-serverstransport-tailnet">`serverstransport.`<br />`tailnet`</a> | Dials the backend servers over the named tailnet from the static configuration, instead of over the host network. Backend names resolve through that tailnet's MagicDNS. <br /> More information [here](../../install-configuration/tailnets.md#backends-over-a-tailnet). | ""      | No       |
