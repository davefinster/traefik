...
| <a id="opt-forwardingTimeouts-readTimeout" href="#opt-forwardingTimeouts-readTimeout" title="#opt-forwardingTimeouts-readTimeout">`forwardingTimeouts.readTimeout`</a> | Defines a timeout for reading a response from the proxied server. The timeout between two successive read operations. The connection is closed if nothing is transmitted within this time.<br />0 = no timeout | 0s      | No       |
| <a id="opt-forwardingTimeouts-writeTimeout" href="#opt-forwardingTimeouts-writeTimeout" title="#opt-forwardingTimeouts-writeTimeout">`forwardingTimeouts.writeTimeout`</a> | Defines a timeout for transmitting a request to the proxied server. The timeout between two successive write operations. The connection is closed if nothing is transmitted within this time.<br />0 = no timeout | 0s      | No       |
...
| <a id="opt-tailnet" href="#opt-tailnet" title="#opt-tailnet">`tailnet`</a> | Dials the backend servers over the named tailnet from the static configuration, instead of over the host network. Backend names resolve through that tailnet's MagicDNS. <br /> More information [here](../../../install-configuration/tailnets.md#backends-over-a-tailnet). | ""      | No       |
