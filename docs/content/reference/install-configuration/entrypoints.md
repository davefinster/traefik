...
| <a id="opt-tailnetListeners-tailnet" href="#opt-tailnetListeners-tailnet" title="#opt-tailnetListeners-tailnet">`tailnetListeners[n].`<br />`tailnet`</a> | The tailnet to accept on, by its name in the install configuration. | -     | Yes      |
| <a id="opt-tailnetListeners-service" href="#opt-tailnetListeners-service" title="#opt-tailnetListeners-service">`tailnetListeners[n].`<br />`service`</a> | Accepts the named Tailscale Service's traffic, on the EntryPoint's port. Mutually exclusive with `address`. | -     | No       |
| <a id="opt-tailnetListeners-address" href="#opt-tailnetListeners-address" title="#opt-tailnetListeners-address">`tailnetListeners[n].`<br />`address`</a> | Where on the tailnet to accept: an IP (typically inside an advertised route) on the EntryPoint's port, an IP and port, or a port alone for the node's own addresses. Mutually exclusive with `service`. | The node's own addresses, on the EntryPoint's port | No       |
