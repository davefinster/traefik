...
| <a id="opt-http2-maxReadFrameSize" href="#opt-http2-maxReadFrameSize" title="#opt-http2-maxReadFrameSize">`http2.`<br />`maxReadFrameSize`</a> | Set the largest frame this endpoint is willing to read. <br /> A valid value is between 16KiB and 16MiB. Zero uses the Go default. | -     | No       |
| <a id="opt-http2-maxReceiveBufferPerConnection" href="#opt-http2-maxReceiveBufferPerConnection" title="#opt-http2-maxReceiveBufferPerConnection">`http2.`<br />`maxReceiveBufferPerConnection`</a> | Set the maximum size of the HTTP/2 flow control window for data received on a connection. <br /> A valid value is at least 64KiB and less than 4MiB. Zero uses the Go default. <br /> This sizes data the endpoint *receives* (request bodies); it does not affect how fast responses are sent, which the client's advertised window governs. | -     | No       |
| <a id="opt-http2-maxReceiveBufferPerStream" href="#opt-http2-maxReceiveBufferPerStream" title="#opt-http2-maxReceiveBufferPerStream">`http2.`<br />`maxReceiveBufferPerStream`</a> | Set the maximum size of the HTTP/2 flow control window for data received on a stream. <br /> A valid value is less than 4MiB. Zero uses the Go default. | -     | No       |
...
| <a id="opt-http3-allow0RTT" href="#opt-http3-allow0RTT" title="#opt-http3-allow0RTT">`http3.allow0RTT`</a> | Controls whether QUIC 0-RTT handshakes are permitted. <br /> This defaults to false because enabling it allows IP allow-lists to be bypassed. <br /> More information [here](#http3). | false | No       |
| <a id="opt-http3-initialPacketSize" href="#opt-http3-initialPacketSize" title="#opt-http3-initialPacketSize">`http3.`<br />`initialPacketSize`</a> | Sets the initial size of, and the lower limit for, packets sent, measured in bytes. <br /> Values below 1200 are invalid per the QUIC specification. When unset, the value is managed by the underlying QUIC library. <br /> Tuning this can be necessary where MTU limits interfere early in the QUIC handshake, before PMTU discovery can engage. | -     | No       |
...
| <a id="opt-tailnet" href="#opt-tailnet" title="#opt-tailnet">`tailnet`</a> | Accepts connections on the named tailnet from the static configuration, instead of on the host network. The address is then interpreted within that tailnet and nothing is bound on the host. <br /> More information [here](./tailnets.md#entrypoints-on-a-tailnet). | -     | No       |
| <a id="opt-tailnetService" href="#opt-tailnetService" title="#opt-tailnetService">`tailnetService`</a> | Accepts the named Tailscale Service's traffic on that tailnet, instead of traffic addressed to the node itself. Requires `tailnet`, a tagged node, and a named port. <br /> More information [here](./tailnets.md#hosting-tailscale-services). | -     | No       |
| <a id="opt-tailnetListeners" href="#opt-tailnetListeners" title="#opt-tailnetListeners">`tailnetListeners`</a> | Additional listeners on tailnets, accepting for this EntryPoint alongside its own `address`, so its routers serve all of them. The EntryPoint's `proxyProtocol` applies to its own address only. <br /> More information [here](./tailnets.md#one-entrypoint-several-listeners). | -     | No       |
| <a id="opt-tailnetListenersn-tailnet" href="#opt-tailnetListenersn-tailnet" title="#opt-tailnetListenersn-tailnet">`tailnetListeners[n].`<br />`tailnet`</a> | The tailnet to accept on, by its name in the install configuration. | -     | Yes      |
| <a id="opt-tailnetListenersn-service" href="#opt-tailnetListenersn-service" title="#opt-tailnetListenersn-service">`tailnetListeners[n].`<br />`service`</a> | Accepts the named Tailscale Service's traffic, on the EntryPoint's port. Mutually exclusive with `address`. | -     | No       |
| <a id="opt-tailnetListenersn-address" href="#opt-tailnetListenersn-address" title="#opt-tailnetListenersn-address">`tailnetListeners[n].`<br />`address`</a> | Where on the tailnet to accept: an IP (typically inside an advertised route) on the EntryPoint's port, an IP and port, or a port alone for the node's own addresses. Mutually exclusive with `service`. | The node's own addresses, on the EntryPoint's port | No       |
...
!!! warning "0-RTT and IP allow-lists"

    Setting `http3.allow0RTT` to `true` (the default is `false`) lets clients replay
    early data, which allows IP allow-lists to be bypassed during the QUIC 0-RTT handshake.
    Only enable it once you have confirmed that nothing downstream relies on such allow-lists.
    More information can be found in
    [GHSA-gxrv-wf35-62w9](https://github.com/traefik/traefik/security/advisories/GHSA-gxrv-wf35-62w9).
